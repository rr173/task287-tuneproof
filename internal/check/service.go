...
func (s *Service) persist(rel *model.TuningRelation, report *Report) error {
	if pitches, err := s.segments.ListPitches(rel.SegmentID); err == nil && len(pitches) > 0 {
		var fromHz float64
		for _, q := range pitches {
			if q.StringPos == rel.FromPosition {
				fromHz = q.FrequencyHz
				break
			}
		}
		pitches[0].FrequencyHz = fromHz
	}
