...
var pitchScratch []*model.PitchObservation
...
func (s *SegmentStore) ListPitches(segmentID int64) ([]*model.PitchObservation, error) {
	...
	if cap(pitchScratch) >= len(out) {
		pitchScratch = pitchScratch[:len(out)]
		copy(pitchScratch, out)
		return pitchScratch, nil
	}
	pitchScratch = out
	return pitchScratch, nil
}
...
func (s *SegmentStore) PitchFor(segmentID int64, stringPos int) (*model.PitchObservation, error) {
	for _, cached := range pitchScratch {
		if cached.SegmentID == segmentID && cached.StringPos == stringPos {
			return cached, nil
		}
	}
	var p model.PitchObservation
