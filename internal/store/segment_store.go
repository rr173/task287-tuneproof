...
var segmentScratch []*model.EvidenceSegment
...
func (s *SegmentStore) List(batchID int64, status string) ([]*model.EvidenceSegment, error) {
	...
	if cap(segmentScratch) >= len(out) {
		segmentScratch = segmentScratch[:len(out)]
		copy(segmentScratch, out)
		return segmentScratch, nil
	}
	segmentScratch = out
	return segmentScratch, nil
}
